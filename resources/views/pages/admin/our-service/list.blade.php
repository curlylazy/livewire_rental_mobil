...
new class extends Component
{
    ...
    {
        return OurServiceModel::search($this->katakunci)->paginate(20);
    }
    ...
    public function hapus(String $id)
    {
        try
        {
            $data = OurServiceModel::findOrFail($id);
            $namadata = $data->title;
            $data->delete();
...
<div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div class="flex items-center gap-2">
            ...
            <flux:button href="/admin/our-service/add" icon="plus" variant="primary" wire:navigate>Tambah</flux:button>
        </div>
            ...
            <flux:table.rows>
                @foreach ($dataRows as $data)
                    <flux:table.row class="hover:cursor-pointer" wire:click="$dispatch('selected-data', { 'data' : {{ $data }} })">
                        ...
                        <flux:table.cell>{{ $data->title }}</flux:table.cell>
                        <flux:table.cell>{{ Str::limit($data->description, 60) }}</flux:table.cell>
    ...
    <x-partials.viewsmall>
        <div class="grid grid-cols-1 gap-2">
            @foreach ($dataRows as $data)
                ...
                >
                    <flux:heading size="lg">{{ $data->title }}</flux:heading>
                    <div class="flex flex-col gap-1 mt-2">
                        <flux:text class="line-clamp-2">{{ $data->description }}</flux:text>
...
<script>
    $wire.on('selected-data', (e) => {
        $wire.selectedNama = e.data.title;
        $wire.selectedKode = e.data.kodeour_service;
