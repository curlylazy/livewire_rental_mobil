...
* **Atribut Model Eksplisit (PHP Attributes):** Karena penamaan model menggunakan akhiran `Model` (misalnya `PaketModel`), konfigurasi model (seperti nama tabel, primary key, guarded, fillable, dll.) wajib menggunakan PHP Attributes dari namespace `Illuminate\Database\Eloquent\Attributes` di atas deklarasi class (contoh: `#[Table(name: 'paket', key: 'kodepaket', keyType: 'string', incrementing: false)]` dan `#[Unguarded]` atau `#[Fillable([...])]`, `#[Hidden([...])]`) alih-alih mendefinisikan properti kelas seperti `$table`, `$primaryKey`, `$keyType`, `$incrementing`, `$guarded`.
* **Rendering View pada Livewire Single File Component:** Jangan gunakan attribute class seperti `#[Title(...)]` or `#[Layout(...)]` untuk mengatur title dan layout view. Lebih baik definisikan secara eksplisit menggunakan method `render()` yang mengembalikan `$this->view()->layout(...)->title(...)` (contoh: `return $this->view()->layout('layouts.app')->title('Login Admin');`).
* **Import Trait & Namespace:** Seluruh import trait (seperti `SoftDeletes`, `HasUuids`, `HasFactory`, dll.) wajib dideklarasikan di bagian atas file menggunakan statement `use` namespace, dan hanya memanggil nama singkat trait di dalam tubuh class (contoh: `use SoftDeletes, HasUuids;`), bukan menuliskan fully qualified name lengkap dengan backslash di dalam class body (contoh larangan: `use \Illuminate\Database\Eloquent\SoftDeletes;` di dalam class).